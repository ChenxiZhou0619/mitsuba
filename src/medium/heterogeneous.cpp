...
public:
  ...
  Float integrateDensity(const Ray &ray) const {
    ...
    Float mint, maxt;
    if (!m_densityAABB.rayIntersect(ray, mint, maxt)) return 0.0f;
    ...
    for (int i = 0; i < 3; ++i)
      maxComp = std::max(std::max(maxComp, std::abs(p[i])), std::abs(pLast[i]));
    if (length < 1e-6f * maxComp) return 0.0f;
...
// Reached the threshold -- stop early
      ...
      if (p == next) {
        Log(EWarn,
            "integrateDensity(): unable to make forward progress -- "
            "round-off error issues? The step size was %e, mint=%f, "
            "maxt=%f, nSteps=%i, ray=%s",
            stepSize, mint, maxt, nSteps, ray.toString().c_str());
        break;
      }
      p = next;
    }
  ...
  bool invertDensityIntegral(const Ray &ray, Float desiredDensity, Float &integratedDensity,
    ...
    /* Determine the ray segment, along which the
       density integration should take place */
    Float mint, maxt;
    if (!m_densityAABB.rayIntersect(ray, mint, maxt)) return false;
    mint         = std::max(mint, ray.mint);
    maxt         = std::min(maxt, ray.maxt);
    Float length = maxt - mint, maxComp = 0;
    Point p = ray(mint), pLast = ray(maxt);
    ...
    for (uint32_t i = 0; i < nSteps; ++i) {
      ...
      if (newDensity >= desiredDensity) {
        ...
        while (true) {
          ...
          if (std::abs(fx) < 1e-6f) {
            ...
            Log(EWarn,
                "invertDensityIntegral(): stuck in Newton-Bisection -- "
      ...
      if (p == next) {
        Log(EWarn,
            "invertDensityIntegral(): unable to make forward progress -- "
            "round-off error issues? The step size was %e",
            stepSize);
        break;
      }
    ...
    if (m_method == ESimpsonQuadrature || sampler == NULL) {
      ...
      for (int i = 0; i < nSamples; ++i) {
        ...
        while (true) {
          ...
          if (density * m_invMaxDensity > sampler->next1D()) break;
        }
      }
