...
public:
   ...
   * \param extraBorder
   *	 Optional extraBorder (used for gradient-domain algorithms to access
   *neighboring pixels)
   */
  ImageBlock(Bitmap::EPixelFormat fmt, const Vector2i &size,
             const ReconstructionFilter *filter = NULL, int channels = -1,
             bool warn = true, int extraborder = 0);

  inline void setAllowNegativeValues(bool b) {
    m_allowNegative = b;
  } // marco: manually allow to accept negative vlaues!
  ...
  inline Bitmap::EPixelFormat getPixelFormat() const {
    return m_bitmap->getPixelFormat();
  }
  ...
  FINLINE bool put(const Point2 &_pos, const Float *value) {
...

    {
      ...
      const Point2 pos(_pos.x - 0.5f - (m_offset.x - m_borderSize),
      ...
      for (int y = min.y, yr = 0; y <= max.y; ++y, ++yr) {
        ...
        Float      *dest =
            m_bitmap->getFloatData() + (y * (size_t)size.x + min.x) * channels;
  ...
  ref<ImageBlock> clone() const {
    ref<ImageBlock> clone = new ImageBlock(
        m_bitmap->getPixelFormat(),
        m_bitmap->getSize() - Vector2i(2 * m_borderSize, 2 * m_borderSize),
        m_filter, m_bitmap->getChannelCount());
    copyTo(clone);
    return clone;
  }
  ...
  void copyTo(ImageBlock *copy) const {
    memcpy(copy->getBitmap()->getUInt8Data(), m_bitmap->getUInt8Data(),
           m_bitmap->getBufferSize());
    copy->m_size   = m_size;
