...
/**
   ...
   since it needs to accumulate sampled data into multiple buffers
   simultaneously.
*/
class GPTWorkResult : public WorkResult {
public:
  const size_t BUFFER_COUNT = 9;
  ...
  inline void put(const Point2 &sample, const Spectrum &spec, float alpha,
                  float weight, int buffer) {
  ...
  inline const Point2i &getOffset() const { return m_block[0]->getOffset(); }
