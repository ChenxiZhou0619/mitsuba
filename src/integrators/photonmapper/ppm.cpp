 ...
 *         in the generated output image (where \code{-1} corresponds to
 $\infty$).
 *         A value of \code{1} will only render directly visible light sources.
 *         \code{2} will lead to single-bounce (direct-only) illumination,
 *         and so on. \default{\code{-1}}
 *     }
 *     \parameter{photonCount}{\Integer}{Number of photons to be shot per
 iteration\default{250000}}
 *     \parameter{initialRadius}{\Float}{Initial radius of gather points in
 world space units.
 *         \default{0, i.e. decide automatically}}
 *     \parameter{alpha}{\Float}{Radius reduction parameter \code{alpha} from
 the paper\default{0.7}}
 *     \parameter{granularity}{\Integer}{
        Granularity of photon tracing work units for the purpose
        of parallelization (in \# of shot particles) \default{0, i.e. decide
 automatically}
 *     }
 ...
 *     }
 *     \parameter{maxPasses}{\Integer}{Maximum number of passes to render (where
 \code{-1}
 *        corresponds to rendering until stopped manually). \default{\code{-1}}}
 * }
 * This plugin implements the progressive photon mapping algorithm by Hachisuka
 et al.
 * \cite{Hachisuka2008Progressive}. Progressive photon mapping is a variant of
 photon
 * mapping that alternates between photon shooting and gathering passes that
 involve
 * a relatively small (e.g. 250K) numbers of photons that are subsequently
 discarded.
 *
 * This is done in a way such that the variance and bias of the resulting output
 * vanish as the number of passes tends to infinity. The progressive nature of
 this
 ...
 * determines how many photon query points are created per pixel. It should not
 be
 ...
 *
 * \remarks{
 *    \item Due to the data dependencies of this algorithm, the parallelization
 is
 *    limited to the local machine (i.e. cluster-wide renderings are not
 implemented)
...
public:
  ...
  PPMIntegrator(const Properties &props) : Integrator(props) {
    /* Initial photon query radius (0 = infer based on scene size and sensor
     * resolution) */
    m_initialRadius = props.getFloat("initialRadius", 0);
    /* Alpha parameter from the paper (influences the speed, at which the photon
     * radius is reduced) */
    ...
    /* Longest visualized path length (<tt>-1</tt>=infinite). When a positive
       value is specified, it must be greater or equal to <tt>2</tt>, which
       corresponds to single-bounce (direct-only) illumination */
    ...
    /* Maximum number of passes to render. -1 renders until the process is
     * stopped. */
    ...
    if (m_maxDepth <= 1 && m_maxDepth != -1)
      Log(EError,
          "Maximum depth must either be set to \"-1\" or \"2\" or higher!");
    if (m_maxPasses <= 0 && m_maxPasses != -1)
      Log(EError, "Maximum number of Passes must either be set to \"-1\" or "
                  "\"1\" or higher!");
  }
  ...
  bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
    ...
    Log(EInfo,
        ...
        " %s, " SSE_STR ") ..",
        cropSize.x, cropSize.y, sampleCount,
    ...
    Log(EInfo, "Creating approximately %i gather points ..",
        cropSize.x * cropSize.y * sampleCount);
    ...
    /* Create gather points in blocks so that gathering can be parallelized
     * later on */
    for (int yofs = 0; yofs < cropSize.y; yofs += blockSize) {
      for (int xofs = 0; xofs < cropSize.x; xofs += blockSize) {
        ...
        wu->block =
            new ImageBlock(Bitmap::ESpectrumAlphaWeight, Vector2i(blockSize),
        ...
        for (int yofsInt = 0; yofsInt < blockSize; ++yofsInt) {
          if (yofsInt + yofs >= cropSize.y) continue;
          for (int xofsInt = 0; xofsInt < blockSize; ++xofsInt) {
            if (xofsInt + xofs >= cropSize.x) continue;
            ...
            for (size_t j = 0; j < sampleCount; j++) {
              ...
              if (needsApertureSample) apertureSample = sensorSampler->next2D();
              if (needsTimeSample) timeSample = sensorSampler->next1D();
              sample.x += x;
              sample.y += y;
              sensor->sampleRayDifferential(sensorRay, sample, apertureSample,
                                            timeSample);
              size_t offset = wu->gatherPoints.size();
              int    count =
                  createGatherPoints(scene, sensorRay, sample, sensorSampler,
                                     Spectrum(1.0f), wu->gatherPoints, 1);
    ...
    while (m_running && (m_maxPasses == -1 || it < m_maxPasses)) {
      photonMapPass(++it, queue, job, film, sceneResID, sensorResID,
                    indepSamplerResID);
    }
  ...
  int createGatherPoints(Scene *scene, const RayDifferential &ray,
    ...
    int count = 0;
    if (depth >= m_maxDepth && m_maxDepth != -1) return 0;
    GatherPoint p;
    ...
    if (scene->rayIntersect(ray, p.its)) {
      ...
      if (bsdf->getType() & BSDF::ESmooth) {
        ...
        p.depth  = depth;
        if (p.its.isEmitter()) p.emission = p.its.Le(-ray.d);
        ...
        for (int i = 0; i < compCount; i++) {
          ...
          if (sampler->next1D() < rrProb) {
            ...
            count += createGatherPoints(scene, recursiveRay, sample, sampler,
                                        weight * bsdfVal / rrProb, gatherPoints,
                                        depth + 1);
          }
        }
      }
  ...
  void photonMapPass(int it, RenderQueue *queue, const RenderJob *job,
                     ...
                     int samplerResID) {
    Log(EInfo,
        "Performing a photon mapping pass %i (" SIZE_T_FMT " photons so far)",
        it, m_totalPhotons);
    ref<Scheduler> sched = Scheduler::getInstance();

    /* Generate the global photon map */
    ref<GatherPhotonProcess> proc = new GatherPhotonProcess(
        GatherPhotonProcess::EAllSurfacePhotons, m_photonCount, m_granularity,
        m_maxDepth == -1 ? -1 : (m_maxDepth - 1), m_rrDepth, true,
    ...
    photonMap->build();
    Log(EDebug,
        "Photon map full. Shot " SIZE_T_FMT
        " particles, excess photons due to parallelism: " SIZE_T_FMT,
    ...
    for (int wuIdx = 0; wuIdx < (int)m_workUnits.size(); ++wuIdx) {
      ...
      for (size_t i = 0; i < wu->gatherPoints.size(); ++i) {
        ...
        Float M = (Float)photonMap->estimateRadianceRaw(
            g.its, g.radius, flux,
            m_maxDepth == -1 ? INT_MAX : (m_maxDepth - g.depth));
        Float N = g.N;
        ...
        contrib =
            g.flux / ((Float)m_totalEmissions * g.radius * g.radius * M_PI) +
            g.emission;
