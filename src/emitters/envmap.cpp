...
public:
  ...
  EnvironmentMap(const Properties &props)
      : Emitter(props), m_mipmap(NULL), m_cdfRows(NULL), m_cdfCols(NULL),
        m_rowWeights(NULL) {
    ...
    if (tryReuseCache &&
      ...
      bool createCache =
          !cacheFile.empty() &&
          props.getBoolean("cache", bitmap->getSize().x * bitmap->getSize().y >
                                        1024 * 1024);
    ...
    if (props.hasProperty("intensityScale"))
      Log(EError, "The 'intensityScale' parameter has been deprecated and is "
                  "now called scale.");
  ...
  EnvironmentMap(Stream *stream, InstanceManager *manager)
      : Emitter(stream, manager), m_mipmap(NULL), m_cdfRows(NULL),
        m_cdfCols(NULL), m_rowWeights(NULL) {
