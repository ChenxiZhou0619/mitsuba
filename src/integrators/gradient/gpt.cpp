...
bool testVisibility(const Scene *scene, const Point3 &point1,
...
VertexType getVertexType(RayState &ray, const GradientPathTracerConfig &config,
                         unsigned int bsdfType) {
...
public:
  GradientPathTracer(const Scene *scene, const Sensor *sensor, Sampler *sampler,
                     GPTWorkResult                  *block,
                     const GradientPathTracerConfig *config)
  ...
  void evaluatePoint(RadianceQueryRecord &rRec, const Point2 &samplePosition,
  ...
  void evaluate(RayState &main, RayState *shiftedRays, int secondaryCount,
                Spectrum &out_veryDirect) {
    const Scene *scene = main.rRec.scene;

    // Perform the first ray intersection for the base path (or ignore if the
    ...
    while (main.rRec.depth < m_config->m_maxDepth || m_config->m_maxDepth < 0) {
      ...
      // Sample incoming radiance from lights (next event estimation).
      {
        ...
        if (main.rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
          ...
          // Calculate the probability density of having generated the sampled
          // path segment by BSDF sampling. Note that if the emitter is not
          // visible, the probability density is zero. Even if the BSDF sampler
          // has zero probability density, the light sampler can still sample
          // it.
          Float mainBsdfPdf =
          ...
          Float mainDistanceSquared =
              (main.rRec.its.p - dRec.p).lengthSquared();
          Float mainOpposingCosine = dot(dRec.n, (main.rRec.its.p - dRec.p)) /
                                     sqrt(mainDistanceSquared);

          // Power heuristic weights for the following strategies: light sample
          // from base, BSDF sample from base.
          ...
          // Strict normals check to produce the same results as bidirectional
          // methods when normal mapping is used.
          if (!m_config->m_strictNormals ||
            ...
            for (int i = 0; i < secondaryCount; ++i) {
              ...
              if (shiftSuccessful) {
                ...
                if (shifted.connection_status == RAY_CONNECTED) {
                  ...
                  mainContribution =
                      main.throughput * (mainBSDFValue * mainEmitterRadiance);
                  shiftedContribution =
                      jacobian * shifted.throughput *
                      (shiftedBsdfValue * shiftedEmitterRadiance);

                  // Note: The Jacobians were baked into shifted.pdf and
                  // shifted.throughput at connection phase.
                } else if (shifted.connection_status ==
                           RAY_RECENTLY_CONNECTED) {
                  // Follow the base path. The current vertex is shared, but the
                  // incoming directions differ.
                  Vector3 incomingDirection =
                      normalize(shifted.rRec.its.p - main.rRec.its.p);

                  BSDFSamplingRecord bRec(
                      main.rRec.its, main.rRec.its.toLocal(incomingDirection),
                      main.rRec.its.toLocal(dRec.d), ERadiance);
                  ...
                  const BSDF *shiftedBSDF =
                      shifted.rRec.its.getBSDF(shifted.ray);

                  // This implementation uses light sampling only for the
                  // reconnect-shift. When one of the BSDFs is very glossy,
                  // light sampling essentially reduces to a failed shift
                  // anyway.
                  ...
                  if (mainAtPointLight ||
                    ...
                    Spectrum shiftedEmitterRadiance =
                        emitterTuple.first * shiftedDRec.pdf;
                    Float shiftedDRecPdf = shiftedDRec.pdf;

                    // Sample the BSDF.
                    Float shiftedDistanceSquared =
                        (dRec.p - shifted.rRec.its.p).lengthSquared();
                    Vector emitterDirection = (dRec.p - shifted.rRec.its.p) /
                                              sqrt(shiftedDistanceSquared);
                    Float shiftedOpposingCosine =
                        -dot(dRec.n, emitterDirection);
                    ...
                    if (m_config->m_strictNormals &&
                      ...
                      Float    shiftedBsdfPdf =
                          (emitter->isOnSurface() &&
                           dRec.measure == ESolidAngle && shiftedEmitterVisible)
                                 ...
                                 : 0;
                      Float jacobian =
                          std::abs(shiftedOpposingCosine *
                                   mainDistanceSquared) /
                          (Epsilon + std::abs(mainOpposingCosine *
                                              shiftedDistanceSquared));

                      // Power heuristic between light sample from base, BSDF
                      // sample from base, light sample from offset, BSDF sample
                      // from offset.
                      Float shiftedWeightDenominator =
                          (jacobian * shifted.pdf) * (jacobian * shifted.pdf) *
                          ((shiftedDRecPdf * shiftedDRecPdf) +
                      ...
                      mainContribution = main.throughput *
                                         (mainBSDFValue * mainEmitterRadiance);
                      shiftedContribution =
                          jacobian * shifted.throughput *
                          (shiftedBsdfValue * shiftedEmitterRadiance);
                    }
              ...
              if (!shiftSuccessful) {
                // The offset path cannot be generated; Set offset PDF and
                // offset throughput to zero. This is what remains.

                // Power heuristic between light sample from base, BSDF sample
                // from base, light sample from offset, BSDF sample from offset.
                // (Offset path has zero PDF)
              ...
              // Note: Using also the offset paths for the throughput estimate,
              // like we do here, provides some advantage when a large
              // reconstruction alpha is used, but using only throughputs of the
              // base paths doesn't usually lose by much.

#ifndef CENTRAL_RADIANCE
              main.addRadiance(mainContribution, weight);
              shifted.addRadiance(shiftedContribution, weight);
#endif
              shifted.addGradient(shiftedContribution - mainContribution,
                                  weight);
            } // for(int i = 0; i < secondaryCount; ++i)
          }   // Strict normals
        }
      ...
      if (m_config->m_strictNormals &&
          mainWoDotGeoN * Frame::cosTheta(mainBsdfResult.bRec.wo) <= 0) {
        break;
      }

      // The old intersection structure is still needed after main.rRec.its gets
      // updated.
      ...
      if (scene->rayIntersect(main.ray, main.rRec.its)) {
        ...
        if (main.rRec.its.hasSubsurface() &&
            (main.rRec.type & RadianceQueryRecord::ESubsurfaceRadiance)) {
          mainEmitterRadiance += main.rRec.its.LoSub(
              scene, main.rRec.sampler, -main.ray.d, main.rRec.depth);
        }
      ...
      // Compute the probability density of generating base path's direction
      // using the implemented direct illumination sampling technique.
      ...
      // Power heuristic weights for the following strategies: light sample from
      // base, BSDF sample from base.
      ...
      if (main.rRec.depth + 1 >= m_config->m_minDepth) {
        main.addRadiance(main.throughput * mainEmitterRadiance,
                         mainWeightNumerator /
                             (D_EPSILON + mainWeightDenominator));
      }
#endif
      ...
      for (int i = 0; i < secondaryCount; ++i) {
        ...
        if (shifted.alive) {
          // The offset path is still good, so it makes sense to continue its
          // construction.
          Float shiftedPreviousPdf = shifted.pdf;

          if (shifted.connection_status == RAY_CONNECTED) {
            // The offset path keeps following the base path.
            // As all relevant vertices are shared, we can just reuse the
            // sampled values.
            ...
            // Power heuristic between light sample from base, BSDF sample from
            // base, light sample from offset, BSDF sample from offset.
            Float shiftedWeightDenominator =
                (shiftedPreviousPdf * shiftedPreviousPdf) *
            ...
            shiftedContribution =
                shifted.throughput *
                shiftedEmitterRadiance; // Note: Jacobian baked into
                                        // .throughput.
          } else if (shifted.connection_status == RAY_RECENTLY_CONNECTED) {
            // Recently connected - follow the base path but evaluate BSDF to
            // the new direction.
            Vector3 incomingDirection =
                normalize(shifted.rRec.its.p - main.ray.o);
            BSDFSamplingRecord bRec(
                previousMainIts, previousMainIts.toLocal(incomingDirection),
                previousMainIts.toLocal(main.ray.d), ERadiance);

            // Note: mainBSDF is the BSDF at previousMainIts, which is the
            // current position of the offset path.

            EMeasure measure = (mainBsdfResult.bRec.sampledType & BSDF::EDelta)
                                   ? EDiscrete
                                   : ESolidAngle;
            ...
            // Not connected - apply either reconnection or half-vector
            // duplication shift.

            const BSDF *shiftedBSDF = shifted.rRec.its.getBSDF(shifted.ray);

            // Update the vertex type of the offset path.
            VertexType shiftedVertexType = getVertexType(
                shifted, *m_config, mainBsdfResult.bRec.sampledType);
            ...
            if (mainVertexType == VERTEX_TYPE_DIFFUSE &&
              ...
              // Optimization: Skip the last raycast and BSDF evaluation for the
              // offset path when it won't contribute and isn't needed anymore.
              if (!lastSegment || mainHitEmitter ||
                ...
                if (main.rRec.its.isValid()) {
                  ...
                  shiftResult = reconnectShift(
                      m_scene, main.ray.o, main.rRec.its.p, shifted.rRec.its.p,
                      main.rRec.its.geoFrame.n, main.ray.time);
                } else {
                  // This is a reconnection at infinity in environment
                  // direction.
                  ...
                  shiftResult =
                      environmentShift(m_scene, main.ray, shifted.rRec.its.p);
                }
                ...
                if (mainHitEmitter || main.rRec.its.hasSubsurface()) {
                  // Also the offset path hit the emitter, as visibility was
                  // checked at reconnectShift or environmentShift.
                  ...
                  if (main.rRec.its.isValid()) {
                    // Hit an object.
                    if (mainHitEmitter) {
                      shiftedEmitterRadiance =
                          main.rRec.its.Le(-outgoingDirection);
                    ...
                    // Sub-surface scattering. Note: Should use the same random
                    // numbers as the base path!
                    if (main.rRec.its.hasSubsurface() &&
                        (main.rRec.type &
                         RadianceQueryRecord::ESubsurfaceRadiance)) {
                      shiftedEmitterRadiance += main.rRec.its.LoSub(
                          scene, shifted.rRec.sampler, -outgoingDirection,
                          main.rRec.depth);
                    }
                  } else {
                    // Hit the environment.
              ...
              // Use half-vector duplication shift. These paths could not have
              ...
              bool bothSmooth =
                  (mainBsdfResult.bRec.sampledType & BSDF::ESmooth) &&
                  (shiftedBSDF->getType() & BSDF::ESmooth);

              auto handle_half_vector_shift_failed = [&]() {
                if (shifted.alive) {
                  // Evaluate radiance difference using power heuristic between
                  // BSDF samples from base and offset paths. Note: No MIS with
                  // light sampling since we don't use it for this connection
                  // type.
                  weight = main.pdf /
                           (shifted.pdf * shifted.pdf + main.pdf * main.pdf);
                  mainContribution = main.throughput * mainEmitterRadiance;
                  shiftedContribution =
                      shifted.throughput *
                      shiftedEmitterRadiance; // Note: Jacobian baked into
                  ...
                  // Handle the failure without taking MIS with light sampling,
                  ...
                  // the gotos... Although this may mean having lots of small
              ...
              if (!(bothDelta || bothSmooth)) {
                ...
                handle_half_vector_shift_failed();
                continue;
              }
              ...
              HalfVectorShiftResult shiftResult = halfVectorShift(
                  mainBsdfResult.bRec.wi, mainBsdfResult.bRec.wo,
                  shifted.rRec.its.toLocal(-shifted.ray.d), mainBSDF->getEta(),
                  shiftedBSDF->getEta());

              if (mainBsdfResult.bRec.sampledType & BSDF::EDelta) {
                // Dirac delta integral is a point evaluation - no Jacobian
                // determinant!
              ...
              Vector3 outgoingDirection =
                  shifted.rRec.its.toWorld(tangentSpaceOutgoingDirection);
              ...
              // Strict normals check to produce the same results as
              // bidirectional methods when normal mapping is used.
              if (m_config->m_strictNormals &&
                  dot(outgoingDirection, shifted.rRec.its.geoFrame.n) *
                          Frame::cosTheta(bRec.wo) <=
                      0) {
              ...
              shifted.ray =
                  Ray(shifted.rRec.its.p, outgoingDirection, main.ray.time);

              if (!scene->rayIntersect(shifted.ray, shifted.rRec.its)) {
                // Hit nothing - Evaluate environment radiance.
                const Emitter *env = scene->getEnvironmentEmitter();
                if (!env) {
                  // Since base paths that hit nothing are not shifted, we must
                  // be symmetric and kill shifts that hit nothing.
                  shifted.alive = false;
                  goto half_vector_shift_failed;
                }
                if (main.rRec.its.isValid()) {
                ...
                if (mainVertexType == VERTEX_TYPE_DIFFUSE &&
                    shiftedVertexType == VERTEX_TYPE_DIFFUSE) {
                  // Environment reconnection shift would have been used for the
                  // reverse direction!
                  shifted.alive = false;
                  goto half_vector_shift_failed;
                }
                ...
                VertexType shiftedNextVertexType = getVertexType(
                    shifted, *m_config, mainBsdfResult.bRec.sampledType);

                // Make sure that the reverse shift would use this same
                // strategy!
                // ==============================================================
                ...
                if (mainVertexType == VERTEX_TYPE_DIFFUSE &&
                  ...
                  // Non-invertible shift: the reverse-shift would use another
                  // strategy!
                  shifted.alive = false;
                  goto half_vector_shift_failed;
                }
                ...
                if (shifted.rRec.its.hasSubsurface() &&
                    (shifted.rRec.type &
                     RadianceQueryRecord::ESubsurfaceRadiance)) {
                  shiftedEmitterRadiance +=
                      shifted.rRec.its.LoSub(scene, shifted.rRec.sampler,
                                             -shifted.ray.d, main.rRec.depth);
                }
              }
            ...
            half_vector_shift_failed:
              if (shifted.alive) {
                ...
                // Handle the failure without taking MIS with light sampling, as
                ...
                // gotos... Although this may mean having lots of small
                // functions with a large number of parameters.)
              }
            }
          }
      ...
      shift_failed:
        ...
        // Note: Using also the offset paths for the throughput estimate, like
        // we do here, provides some advantage when a large reconstruction alpha
        // is used, but using only throughputs of the base paths doesn't usually
        // lose by much.
        if (main.rRec.depth + 1 >= m_config->m_minDepth) {
#ifndef CENTRAL_RADIANCE
          main.addRadiance(mainContribution, weight);
