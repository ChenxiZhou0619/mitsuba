...
struct Frame {
  ...
  inline Frame(const Vector &n) : n(n) { coordinateSystem(n, s, t); }
  ...
  inline static Float cosTheta2(const Vector &v) { return v.z * v.z; }

  /** \brief Assuming that the given direction is in the local coordinate
   * system, return the cosine of the angle between the normal and v */
  inline static Float cosTheta(const Vector &v) { return v.z; }

  /** \brief Assuming that the given direction is in the local coordinate
   * system, return the u and v coordinates of the vector 'v' */
  inline static Vector2 uv(const Vector &v) { return Vector2(v.x, v.y); }

  /** \brief Assuming that the given direction is in the local coordinate
   * system, return the squared sine of the angle between the normal and v */
  inline static Float sinTheta2(const Vector &v) { return 1.0f - v.z * v.z; }
  ...
  /// Return theta phi of the local vector
  inline static std::pair<Float, Float> toThetaPhi(const Vector &v) {
    Float theta   = std::acos(v.z);
    Float tan_phi = v.y / (v.x + 1e-7f);
    Float phi     = std::atan(tan_phi);
