...
public:
   ...
   */
  Spectrum sampleEmitterDirect(DirectSamplingRecord &dRec, const Point2 &sample,
   ...
   *    the sampled ray divided by the sample probability.
   */
  Spectrum sampleSensorDirect(DirectSamplingRecord &dRec, const Point2 &sample,
                              bool testVisibility = true) const;
  ...
  inline Spectrum evalAttenuatedEnvironment(const RayDifferential &ray, const Medium *medium,
    ...
    if (medium) result *= medium->evalTransmittance(ray, sampler);
    return result;
  }
  ...
  inline bool destinationExists() const {
