 ...
 *         For the \code{spectrum*} options, the number of written channels
 * depends on the value assigned to \code{SPECTRUM\_SAMPLES} during compilation
 ...
 * }
 *
 * This film allows to write several HDR output images at once.  Currently it is
 * only used for gradient-domain (bidirectional) path tracing (\pluginref{gpt}
 * and \pluginref{gbdpt}). This Film is automatically selected when switching to
 * G-PT or G-BDPT in the GUI. MultiFilm has the exact same parameters as
 * HDRFilm.
 *
 */
...
public:
  ...
  bool develop(const Point2i &offset, const Vector2i &size,
  ...
  void put(const ImageBlock *bitmap) { putMulti(bitmap, 0); }
  ...
  MultiFilm(const Properties &props) : Film(props) {
    ...
    std::string fileFormat =
        boost::to_lower_copy(props.getString("fileFormat", "openexr"));
    ...
    if ((pixelFormats.size() != 1 &&
      ...
      Log(EError, "Number of channel names must match the number of specified "
                  "pixel formats!");

    if (pixelFormats.size() != 1 && m_fileFormat != Bitmap::EOpenEXR)
      Log(EError, "General multi-channel output is only supported when writing "
                  "OpenEXR files!");

    for (size_t i = 0; i < pixelFormats.size(); ++i) {
      std::string pixelFormat = pixelFormats[i];
      std::string name =
          i < channelNames.size() ? (channelNames[i] + std::string(".")) : "";
      ...
      if (pixelFormat == "luminance") {
        ...
        for (int i = 0; i < SPECTRUM_SAMPLES; ++i) {
          ...
          m_channelNames.push_back(name + formatString("%.2f-%.2fnm",
                                                       coverage.first,
                                                       coverage.second));
        }
    ...
    for (size_t i = 0; i < m_pixelFormats.size(); ++i) {
      if (SPECTRUM_SAMPLES == 3 &&
        ...
        Log(EError, "You requested to render a spectral image, but Mitsuba is "
                    "currently "
    ...
    if (m_fileFormat == Bitmap::ERGBE) {
      ...
      if (m_pixelFormats.size() != 1)
        Log(EError,
            "The RGBE format does not support general multi-channel images!");
      if (m_pixelFormats[0] != Bitmap::ERGB) {
        Log(EWarn,
            "The RGBE format only supports pixelFormat=\"rgb\". Overriding..");
        m_pixelFormats[0] = Bitmap::ERGB;
      }
      if (m_componentFormat != Bitmap::EFloat32) {
        Log(EWarn, "The RGBE format only supports componentFormat=\"float32\". "
                   "Overriding..");
        m_componentFormat = Bitmap::EFloat32;
      }
      ...
      if (m_pixelFormats.size() != 1)
        Log(EError,
            "The PFM format does not support general multi-channel images!");
      if (m_pixelFormats[0] != Bitmap::ERGB &&
          m_pixelFormats[0] != Bitmap::ELuminance) {
        Log(EWarn,
            "The PFM format only supports pixelFormat=\"rgb\" or \"luminance\"."
            " Overriding (setting to \"rgb\")..");
        m_pixelFormats[0] = Bitmap::ERGB;
      }
      if (m_componentFormat != Bitmap::EFloat32) {
        Log(EWarn, "The PFM format only supports componentFormat=\"float32\". "
                   "Overriding..");
        m_componentFormat = Bitmap::EFloat32;
      }
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
      ...
      if ((boost::starts_with(key, "metadata['") &&
           boost::ends_with(key, "']")) ||
    ...
    /*	m_numBuffers = 1;
            ...
            if (m_pixelFormats.size() == 1) {
                    m_storage[0] = new ImageBlock(Bitmap::ESpectrumAlphaWeight,
       m_cropSize); } else { m_storage[0] = new
       ImageBlock(Bitmap::EMultiSpectrumAlphaWeight, m_cropSize, NULL, (int)
       (SPECTRUM_SAMPLES * m_pixelFormats.size() + 2));
            }
  ...
  MultiFilm(Stream *stream, InstanceManager *manager) : Film(stream, manager) {
  ...
  bool setBuffers(std::vector<std::string> &names) {
    ...
    for (size_t i = 0; i < m_numBuffers; ++i) {
      m_storage[i]  = new ImageBlock(Bitmap::ESpectrumAlphaWeight, m_cropSize,
                                     NULL, -1, true);
      m_ext_name[i] = std::string(names.at(i));
    }
  ...
  void addBitmapMulti(const Bitmap *bitmap, Float multiplier, int buf) {
    ...
    if ((bitmap->getPixelFormat() != Bitmap::ESpectrum &&
         bitmap->getPixelFormat() != Bitmap::ESpectrumAlphaWeight) ||
        bitmap->getComponentFormat() != Bitmap::EFloat ||
        bitmap->getGamma() != 1.0f || size != m_storage[buf]->getSize() ||
    ...
    if (hasAlpha) {
      ...
      for (size_t i = 0; i < nPixels; ++i) {
        ...
        if (weight == 0) weight = target[SPECTRUM_SAMPLES + 1] = 1;
        weight *= multiplier;
        for (size_t j = 0; j < SPECTRUM_SAMPLES; ++j)
  ...
  bool developMulti(const Point2i &sourceOffset, const Vector2i &size,
                    const Point2i &targetOffset, Bitmap *target,
                    int buf) const {
    if (buf >= m_numBuffers) return false;
    ...
    if (EXPECT_NOT_TAKEN(m_pixelFormats.size() != 1)) {
      /* Special case for general multi-channel images -- just develop the first
       * component(s) */
      for (int i = 0; i < size.y; ++i) {
        for (int j = 0; j < size.x; ++j) {
          Float weight =
              *((Float *)(sourceData + (j + 1) * sourceBpp - sizeof(Float)));
          Float invWeight = weight != 0 ? ((Float)1 / weight) : (Float)0;
          cvt->convert(Bitmap::ESpectrum, 1.0f, sourceData + j * sourceBpp,
                       target->getPixelFormat(), target->getGamma(),
                       targetData + j * targetBpp, 1, invWeight);
        }
    ...
    } else if (size.x == m_cropSize.x &&
               target->getWidth() == m_storage[0]->getWidth()) {
  ...
  void develop(const Scene *scene, Float renderTime) {
    ...
    for (int buf = 0; buf < m_numBuffers; buf++) {
      ref<Bitmap> bitmap;
      if (m_pixelFormats.size() == 1) {
        bitmap = m_storage[buf]->getBitmap()->convert(m_pixelFormats[0],
                                                      m_componentFormat);
        bitmap->setChannelNames(m_channelNames);
      } else {
        bitmap = m_storage[buf]->getBitmap()->convertMultiSpectrumAlphaWeight(
            m_pixelFormats, m_componentFormat, m_channelNames);
      }

      if (m_banner && m_cropSize.x > bannerWidth + 5 &&
          m_cropSize.y > bannerHeight + 5 && m_pixelFormats.size() == 1) {
        ...
        for (int y = 0; y < bannerHeight; y++) {
          for (int x = 0; x < bannerWidth; x++) {
            if (banner[x + y * bannerWidth]) continue;
      ...
      filename /= std::string(m_destFile.filename().string().c_str()) +
                  m_ext_name.at(buf).c_str();
    ...
    if (1) {
      ...
      if (logger->readLog(log)) {
        ...
        filename /=
            std::string(m_destFile.filename().string().c_str()) + "-log";
      ...
      filename.remove_filename();
      filename /=
          std::string(m_destFile.filename().string().c_str()) + "-stats";
      filename.replace_extension(".txt");
