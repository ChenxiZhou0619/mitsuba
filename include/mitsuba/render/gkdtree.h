...
public:
  ...
  template <typename T> void release(T *ptr) {
    ...
    for (std::vector<Chunk>::iterator it = m_chunks.begin();
      ...
      if ((uint8_t *)ptr == chunk.start + chunk.size) return;
    }
...
private:
  struct Chunk {
    ...
    inline size_t used() const { return cur - start; }

    inline size_t remainder() const { return size - used(); }

    std::string toString() const {
      return formatString("0x%llx-0x%llx (size=" SIZE_T_FMT ", used=" SIZE_T_FMT
                          ")",
                          start, start + size, size, used());
    }
  };
...
public:
  ...
  inline void push_back(const T &value) {
    ...
    if (blockIdx == m_blocks.size()) m_blocks.push_back(new T[BlockSize]);
  ...
  inline T &operator[](size_t index) {
    return *(m_blocks[index / BlockSize] + (index % BlockSize));
  }
   ...
   */
  inline size_t blockCount() const { return m_blocks.size(); }

  /**
   * \brief Return the total capacity
   */
  inline size_t capacity() const { return m_blocks.size() * BlockSize; }
...
public:
  ClassificationStorage() : m_buffer(NULL), m_bufferSize(0) {}
...
public:
  ...
  struct KDNode {
    ...
    inline bool initInnerNode(int axis, float split, ptrdiff_t relOffset) {
      if (relOffset < 0 || relOffset > ERelOffsetLimit) return false;
    ...
    inline void initIndirectionNode(int axis, float split,
      ...
      inner.combined =
          EIndirectionMask | ((uint32_t)indirectionEntry << 2) | axis;
      inner.split = split;
    }

    /// Is this a leaf node?
    FINLINE bool isLeaf() const { return leaf.combined & (uint32_t)ETypeMask; }
    ...
    FINLINE IndexType getPrimEnd() const { return leaf.end; }
    ...
    FINLINE const KDNode *__restrict getLeft() const {
      return this + ((inner.combined & (uint32_t)EInnerOffsetMask) >> 2);
    }

    /// Return the sibling of the current node
    FINLINE const KDNode *__restrict getSibling() const {
      return (const KDNode *)((ptrdiff_t)this ^
                              (ptrdiff_t)8); // comment : beautiful
    }
    ...
    /// Return the left child (assuming that this is an interior node)
    FINLINE const KDNode *__restrict getRight() const { return getLeft() + 1; }

    /**
     * \brief Return the split plane location (assuming that this
     * is an interior node)
     */
    FINLINE float getSplit() const { return inner.split; }
    ...
    std::string toString() const {
      ...
      if (isLeaf()) {
        ...
        oss << "KDNode[interior, axis=" << getAxis() << ", split=" << getAxis()
  ...
  inline const KDNode *getRoot() const { return m_nodes; }

  /// Return whether or not the kd-tree has been built
  inline bool isBuilt() const { return m_nodes != NULL; }

  /// Return a (slightly enlarged) axis-aligned bounding box containing all
  /// primitives
...
#define KDLog(level, fmt, ...)                                                 \
  Thread::getThread()->getLogger()->log(level, KDTreeBase<AABB>::m_theClass,   \
 ...
 * \ingroup librender
 */
template <typename AABBType, typename TreeConstructionHeuristic,
          typename Derived>
...
public:
  ...
  virtual ~GenericKDTree() {
    ...
    if (m_nodes) freeAligned(m_nodes - 1); // undo alignment shift
  }
   ...
   */
  inline void setQueryCost(Float queryCost) { m_queryCost = queryCost; }
   ...
   *  a kd-tree search query)
   */
  inline Float getQueryCost() const { return m_queryCost; }
   ...
   * tree construction heuristic
   */
  inline Float getEmptySpaceBonus() const { return m_emptySpaceBonus; }

  /**
   * \brief Set the maximum tree depth (0 = use heuristic)
   */
  inline void setMaxDepth(SizeType maxDepth) { m_maxDepth = maxDepth; }

  /**
   * \brief Set the number of bins used for Min-Max binning
   */
  inline void setMinMaxBins(SizeType minMaxBins) { m_minMaxBins = minMaxBins; }

  /**
   * \brief Return the number of bins used for Min-Max binning
   */
  inline SizeType getMinMaxBins() const { return m_minMaxBins; }

  /**
   * \brief Return maximum tree depth (0 = use heuristic)
   */
  inline SizeType getMaxDepth() const { return m_maxDepth; }

  /**
   * \brief Specify whether or not to use primitive clipping will
   * be used in the tree construction.
   */
  inline void setClip(bool clip) { m_clip = clip; }
   ...
   */
  inline void setRetract(bool retract) { m_retract = retract; }

  /**
   * \brief Return whether or not bad splits can be "retracted".
   */
  inline bool getRetract() const { return m_retract; }
   ...
   * in succession before a leaf node will be created.
   */
  inline SizeType getMaxBadRefines() const { return m_maxBadRefines; }

  /**
   * \brief Set the number of primitives, at which recursion will
   * stop when building the tree.
   */
  inline void setStopPrims(SizeType stopPrims) { m_stopPrims = stopPrims; }

  /**
   * \brief Return the number of primitives, at which recursion will
   * stop when building the tree.
   */
  inline SizeType getStopPrims() const { return m_stopPrims; }

  /**
   * \brief Specify whether or not tree construction
   * should run in parallel.
   */
  inline void setParallelBuild(bool parallel) { m_parallelBuild = parallel; }

  /**
   * \brief Return whether or not tree construction
   * will run in parallel.
   */
  inline bool getParallelBuild() const { return m_parallelBuild; }
...
protected:
  ...
  void buildInternal() {
    /* Some samity checks */
    if (isBuilt()) KDLog(EError, "The kd-tree has already been built!");
    if (m_traversalCost <= 0) KDLog(EError, "The traveral cost must be > 0");
    if (m_queryCost <= 0) KDLog(EError, "The query cost must be > 0");
    ...
    if (primCount <= m_exactPrimThreshold) m_parallelBuild = false;

    BuildContext ctx(primCount, m_minMaxBins);

    /* Establish an ad-hoc depth cutoff value (Formula from PBRT) */
    if (m_maxDepth == 0) m_maxDepth = (int)(8 + 1.3f * math::log2i(primCount));
    ...
    KDLog(m_logLevel, "   Perfect splits           : %s",
          m_clip ? "yes" : "no");
    ...
    if (procCount == 1) m_parallelBuild = false;
    ...
    buildTreeMinMax(ctx, 1, prelimRoot, aabb, aabb, indices, primCount, true,
                    0);
    ...
    KDLog(m_logLevel, "   Indirection entries    : " SIZE_T_FMT " (%s)",
          m_indirections.size(),
          memString(m_indirections.capacity() * sizeof(KDNode *)).c_str());

    KDLog(m_logLevel, "   Main thread:");
    ctx.printStats(m_logLevel);
    size_t totalUsage =
        m_indirections.capacity() * sizeof(KDNode *) + ctx.size();
    ...
    // +1 shift is for alignment purposes (see KDNode::getSibling)
    m_nodes = static_cast<KDNode *>(
                  allocAligned(sizeof(KDNode) * (m_nodeCount + 1))) +
              1;
    ...
    while (!stack.empty()) {
      ...
      if (item.node->isLeaf()) {
        ...
        if (primsInLeaf < primBucketCount) primBuckets[primsInLeaf]++;
        if (primsInLeaf > maxPrimsInLeaf) maxPrimsInLeaf = primsInLeaf;
        ...
        bool  result =
            item.target->initInnerNode(axis, split, children - item.target);
        if (!result)
        ...
        stack.push(
            RewriteItem(left + 1, children + 1, item.context, item.aabb));
    ...
    KDLog(m_logLevel, "   Inner nodes                 : %i",
          ctx.innerNodeCount);
    ...
    KDLog(m_logLevel, "   Retracted splits            : %i",
          ctx.retractedSplits);
    ...
    KDLog(m_logLevel, "   Expected traversals/query   : %.2f",
          expTraversalSteps);
    KDLog(m_logLevel, "   Expected leaf visits/query  : %.2f",
          expLeavesVisited);
...
protected:
  ...
  struct EdgeEventOrdering
      : public std::binary_function<EdgeEvent, EdgeEvent, bool> {
  ...
  struct SplitCandidate {
    ...
    inline SplitCandidate()
        : cost(std::numeric_limits<Float>::infinity()), pos(0), axis(0),
          numLeft(0), numRight(0), planarLeft(false), leftBin(-1) {}
  ...
  struct BuildContext {
    ...
    BuildContext(SizeType primCount, SizeType binCount) : minMaxBins(binCount) {
    ...
    size_t size() {
      return leftAlloc.size() + rightAlloc.size() +
             ...
             indices.capacity() * sizeof(IndexType) + classStorage.size();
    }
    ...
    void printStats(ELogLevel level) {
      ...
      KDLog(level,
            "      kd-tree nodes : " SIZE_T_FMT " entries, " SIZE_T_FMT
            " blocks (%s)",
            nodes.size(), nodes.blockCount(),
            memString(nodes.capacity() * sizeof(KDNode)).c_str());
      KDLog(level,
            "      Indices       : " SIZE_T_FMT " entries, " SIZE_T_FMT
            " blocks (%s)",
            indices.size(), indices.blockCount(),
            memString(indices.capacity() * sizeof(IndexType)).c_str());
    }
  ...
  public:
    TreeBuilder(IndexType id, GenericKDTree *parent)
        : Thread(formatString("bld%i", id)), m_id(id), m_parent(parent),
    ...
    void run() {
      ...
      while (true) {
        ...
        m_parent->buildTree(m_context, depth, node, nodeAABB, eventStart,
                            eventEnd, primCount, true, badRefines);
        leftAlloc.release(eventStart);
      }
    }

    inline BuildContext &getContext() { return m_context; }
   ...
   */
  EventList createEventList(OrderedChunkAllocator &alloc,
                            const AABBType &nodeAABB, IndexType *prims,
                            SizeType primCount) {
    ...
    for (SizeType i = 0; i < primCount; ++i) {
      ...
      if (m_clip) {
        ...
        if (!aabb.isValid() || aabb.getSurfaceArea() == 0) continue;
      } else {
  ...
  void createLeaf(BuildContext &ctx, KDNode *node, EdgeEvent *eventStart,
    ...
    if (primCount > 0) {
      ...
      for (EdgeEvent *event = eventStart; event != eventEnd && event->axis == 0;
           ++event) {
  ...
  void createLeafAfterRetraction(BuildContext &ctx, KDNode *node,
    ...
    IndexType *tempStart = alloc.allocate<IndexType>(indexCount),
              *tempEnd = tempStart + indexCount, *ptr = tempStart;
   ...
   */
  inline Float transitionToNLogN(BuildContext &ctx, unsigned int depth,
                                 KDNode *node, const AABBType &nodeAABB,
                                 IndexType *indices, SizeType primCount,
                                 bool isLeftChild, SizeType badRefines) {
    ...
    if (m_parallelBuild) {
      ...
      cost = buildTree(ctx, depth, node, nodeAABB, events.start, events.end,
                       events.primCount, isLeftChild, badRefines);
    }
  ...
  Float buildTreeMinMax(BuildContext &ctx, unsigned int depth, KDNode *node,
                        const AABBType &nodeAABB, const AABBType &tightAABB,
                        IndexType *indices, SizeType primCount,
                        bool isLeftChild, SizeType badRefines) {
    ...
    if (primCount <= m_exactPrimThreshold)
      return transitionToNLogN(ctx, depth, node, nodeAABB, indices, primCount,
    ...
    SplitCandidate bestSplit =
        ctx.minMaxBins.minimizeCost(m_traversalCost, m_queryCost);
    ...
    if (bestSplit.cost == std::numeric_limits<Float>::infinity()) {
      /* This is bad: we have either run out of floating point precision to
         ...
         along an axis), or the compiler made overly liberal use of floating
         point optimizations, causing the two stages of the min-max binning code
         to become inconsistent. The two ways to proceed at this point are to
    ...
    typename MinMaxBins::Partition partition =
        ctx.minMaxBins.partition(ctx, cast(), indices, bestSplit, isLeftChild,
                                 m_traversalCost, m_queryCost);
    ...
    Float leftCost = buildTreeMinMax(ctx, depth + 1, children, childAABB,
                                     partition.left, partition.leftIndices,
    ...
    Float rightCost = buildTreeMinMax(ctx, depth + 1, children + 1, childAABB,
                                      partition.right, partition.rightIndices,
                                      bestSplit.numRight, false, badRefines);

    TreeConstructionHeuristic tch(nodeAABB);
    std::pair<Float, Float>   prob =
        tch(bestSplit.axis, bestSplit.pos - nodeAABB.min[bestSplit.axis],
            nodeAABB.max[bestSplit.axis] - bestSplit.pos);

    /* Compute the final cost given the updated cost
       values received from the children */
    Float finalCost =
        m_traversalCost + (prob.first * leftCost + prob.second * rightCost);
  ...
  Float buildTree(BuildContext &ctx, unsigned int depth, KDNode *node,
                  const AABBType &nodeAABB, EdgeEvent *eventStart,
                  EdgeEvent *eventEnd, SizeType primCount, bool isLeftChild,
                  SizeType badRefines) {
    ...
    SizeType numLeft[PointType::dim], numRight[PointType::dim];
    ...
    for (EdgeEvent *event = eventStart; event < eventEnd;) {
      ...
      if (EXPECT_TAKEN(pos > nodeAABB.min[axis] && pos < nodeAABB.max[axis])) {
        ...
        std::pair<Float, Float> prob =
            tch(axis, pos - nodeAABB.min[axis], nodeAABB.max[axis] - pos);

        if (numPlanar == 0) {
          Float cost = m_traversalCost +
                       m_queryCost * (prob.first * nLF + prob.second * nRF);
          if (nL == 0 || nR == 0) cost *= m_emptySpaceBonus;
    ...
    for (int i = 1; i < PointType::dim; ++i)
      KDAssert(eventsByAxis[i]->axis == i &&
               (eventsByAxis[i] - 1)->axis == i - 1);
#endif
    ...
    if (isLeftChild) {
      ...
      rightEventsStart = rightAlloc.allocate<EdgeEvent>(bestSplit.numRight * 2 *
                                                        PointType::dim);
    } else {
      leftEventsStart =
          leftAlloc.allocate<EdgeEvent>(bestSplit.numLeft * 2 * PointType::dim);
    ...
    if (m_clip) {
      EdgeEvent *leftEventsTempStart = leftAlloc.allocate<EdgeEvent>(
                    primsLeft * 2 * PointType::dim),
                *rightEventsTempStart = rightAlloc.allocate<EdgeEvent>(
                    primsRight * 2 * PointType::dim),
                *newEventsLeftStart = leftAlloc.allocate<EdgeEvent>(
                    primsBoth * 2 * PointType::dim),
                *newEventsRightStart = rightAlloc.allocate<EdgeEvent>(
                    primsBoth * 2 * PointType::dim);
        ...
        if (classification == ELeftSide) {
          ...
          *rightEventsTempEnd++ = *event;
          ...
          if (clippedLeft.isValid() && clippedLeft.getSurfaceArea() > 0) {
            for (int axis = 0; axis < PointType::dim; ++axis) {
              float min = clippedLeft.min[axis], max = clippedLeft.max[axis];

              if (min == max) {
                *newEventsLeftEnd++ =
                    EdgeEvent(EdgeEvent::EEdgePlanar, axis, min, index);
              } else {
                *newEventsLeftEnd++ =
                    EdgeEvent(EdgeEvent::EEdgeStart, axis, min, index);
                *newEventsLeftEnd++ =
                    EdgeEvent(EdgeEvent::EEdgeEnd, axis, max, index);
              }
            }
          } else {
            prunedLeft++;
          ...
          if (clippedRight.isValid() && clippedRight.getSurfaceArea() > 0) {
            for (int axis = 0; axis < PointType::dim; ++axis) {
              float min = clippedRight.min[axis], max = clippedRight.max[axis];
      ...
      KDAssert((SizeType)(leftEventsTempEnd - leftEventsTempStart) <=
               primsLeft * 2 * PointType::dim);
      KDAssert((SizeType)(rightEventsTempEnd - rightEventsTempStart) <=
               primsRight * 2 * PointType::dim);
      KDAssert((SizeType)(newEventsLeftEnd - newEventsLeftStart) <=
               primsBoth * 2 * PointType::dim);
      KDAssert((SizeType)(newEventsRightEnd - newEventsRightStart) <=
               primsBoth * 2 * PointType::dim);
      ...
      /* Merge the left list */
      leftEventsEnd =
          std::merge(leftEventsTempStart, leftEventsTempEnd, newEventsLeftStart,
                     newEventsLeftEnd, leftEventsStart, EdgeEventOrdering());

      /* Merge the right list */
      rightEventsEnd = std::merge(rightEventsTempStart, rightEventsTempEnd,
      ...
      KDAssert((SizeType)(leftEventsEnd - leftEventsStart) <=
               bestSplit.numLeft * 2 * PointType::dim);
      KDAssert((SizeType)(rightEventsEnd - rightEventsStart) <=
               bestSplit.numRight * 2 * PointType::dim);
    }
    ...
    Float leftCost = buildTree(
        ctx, depth + 1, children, leftNodeAABB, leftEventsStart, leftEventsEnd,
        bestSplit.numLeft - prunedLeft, true, badRefines);

    Float rightCost = buildTree(
        ctx, depth + 1, children + 1, rightNodeAABB, rightEventsStart,
        rightEventsEnd, bestSplit.numRight - prunedRight, false, badRefines);
  ...
  struct MinMaxBins {
    ...
    inline IndexType computeIndex(float pos, int axis) {
      return (IndexType)std::min(
          (float)(m_binCount - 1),
          std::max(0.0f, (pos - m_min[axis]) * m_invBinSize[axis]));
    }
      ...
      for (SizeType i = 0; i < m_primCount; ++i) {
        ...
        for (int axis = 0; axis < PointType::dim; ++axis) {
          m_minBins[axis * m_binCount +
                    computeIndex(math::castflt_down(aabb.min[axis]), axis)]++;
          m_maxBins[axis * m_binCount +
                    computeIndex(math::castflt_up(aabb.max[axis]), axis)]++;
        }
      }
    }
    ...
    SplitCandidate minimizeCost(Float traversalCost, Float queryCost) {
      ...
      for (int axis = 0; axis < PointType::dim; ++axis) {
        ...
        for (int i = 0; i < m_binCount - 1; ++i) {
          ...
          leftWidth += binSize;
          rightWidth -= binSize;
          std::pair<Float, Float> prob = tch(axis, leftWidth, rightWidth);

          Float cost = traversalCost + queryCost * (prob.first * numLeft +
                                                    prob.second * numRight);
     ...
     * primitive lists.
     */
    Partition partition(BuildContext &ctx, const Derived *derived,
                        IndexType *primIndices, SplitCandidate &split,
                        bool isLeftChild, Float traversalCost,
                        Float queryCost) {
      ...
      return Partition(leftBounds, leftIndices, rightBounds, rightIndices);
    }
