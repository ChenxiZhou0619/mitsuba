...
GPTWorkResult::GPTWorkResult(const ReconstructionFilter *rfilter,
                             Vector2i blockSize, int extraBorder) {
  if (blockSize == Vector2i(-1, -1)) blockSize = Vector2i(32, 32);

  m_block.resize(
      BUFFER_COUNT); // 0: preview, 1: throughput, 2: dx, 3: dy, 4: very direct

  for (size_t i = 0; i < m_block.size(); ++i) {
    m_block[i] = new ImageBlock(Bitmap::ESpectrumAlphaWeight, blockSize,
                                rfilter, -1, true, extraBorder);
