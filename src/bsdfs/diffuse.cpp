 ...
 * parameterization. When no parameters are specified, the model uses the
 * default of 50% reflectance.
 *
...
public:
  SmoothDiffuse(const Properties &props) : BSDF(props) {
    ...
    m_reflectance = new ConstantSpectrumTexture(props.getSpectrum(
        props.hasProperty("reflectance") ? "reflectance" : "diffuseReflectance",
        Spectrum(.5f)));
  }
  ...
  void configure() {
    ...
    m_reflectance =
        ensureEnergyConservation(m_reflectance, "reflectance", 1.0f);

    m_components.clear();
    if (m_reflectance->getMaximum().max() > 0)
      m_components.push_back(
          EDiffuseReflection | EFrontSide |
  ...
  Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
    ...
    return m_reflectance->eval(bRec.its) * (INV_PI * Frame::cosTheta(bRec.wo));
  }
...
public:
  SmoothDiffuseShader(Renderer *renderer, const Texture *reflectance)
    ...
    m_reflectanceShader =
        renderer->registerShaderForResource(m_reflectance.get());
  }

  bool isComplete() const { return m_reflectanceShader.get() != NULL; }
  ...
  void generateCode(std::ostringstream &oss, const std::string &evalName,
    ...
    oss << "vec3 " << evalName << "(vec2 uv, vec3 wi, vec3 wo) {" << endl
        ...
        << "    return " << depNames[0] << "(uv) * inv_pi * cosTheta(wo);"
        << endl
        << "}" << endl
        << endl
