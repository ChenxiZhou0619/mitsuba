...
Shape::Shape(const Properties &props) : ConfigurableObject(props) { m_name = props.getID(); }

Shape::Shape(Stream *stream, InstanceManager *manager) : ConfigurableObject(stream, manager) {
...
void Shape::configure() {
  if (m_bsdf == NULL) {
    ...
    if (isEmitter() || isSensor() || hasSubsurface()) {
      ...
      bsdf =
          static_cast<BSDF *>(PluginManager::getInstance()->createObject(MTS_CLASS(BSDF), props));
      ...
      bsdf = static_cast<BSDF *>(
          PluginManager::getInstance()->createObject(MTS_CLASS(BSDF), Properties("null")));
    }
    bsdf->configure();
    addChild(bsdf);
  }
  ...
  if ((m_bsdf->getType() & BSDF::ENull) && (isEmitter() || isSensor() || hasSubsurface()))
    Log(EError,
        "Shape \"%s\" has an index-matched BSDF and an "
        "emitter/sensor/subsurface attachment. This is not allowed!",
        getName().c_str());
}
...
bool Shape::isCompound() const { return false; }

std::string Shape::getName() const { return m_name; }

Shape *Shape::getElement(int i) { return NULL; }
...
void Shape::sampleDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
...
void Shape::addChild(const std::string &name, ConfigurableObject *child) {
  ...
  if (cClass->derivesFrom(MTS_CLASS(BSDF))) {
    ...
    if (m_emitter != NULL) Log(EError, "Tried to attach multiple emitters to a shape!");
    if (emitter) {
      if (!emitter->isOnSurface())
        Log(EError, "Tried to attach an incompatible emitter to a surface!");
      if (m_exteriorMedium) emitter->setMedium(m_exteriorMedium);
    }
    m_emitter = emitter;
  } else if (cClass->derivesFrom(MTS_CLASS(Sensor))) {
    Sensor *sensor = static_cast<Sensor *>(child);
    if (m_sensor != NULL) Log(EError, "Tried to attach multiple sensors to a shape!");
    if (sensor) {
      if (!sensor->isOnSurface())
        Log(EError, "Tried to attach an incompatible sensor to a surface!");
      if (m_exteriorMedium) sensor->setMedium(m_exteriorMedium);
    }
    m_sensor = sensor;
    ...
    if (m_interiorMedium != NULL)
      Log(EError,
          "Shape \"%s\" has both an interior medium "
          "and a subsurface scattering model -- please choose one or the other!",
          getName().c_str());
    ...
    if (name == "interior") {
      ...
      if (m_emitter) m_emitter->setMedium(m_exteriorMedium);
      if (m_sensor) m_sensor->setMedium(m_exteriorMedium);
    } else {
...
const KDTreeBase<AABB> *Shape::getKDTree() const { return NULL; }
...
bool  Shape::rayIntersect(const Ray &ray, Float mint, Float maxt, Float &t, void *temp) const {
  NotImplementedError("rayIntersect");
}
bool Shape::rayIntersect(const Ray &ray, Float mint, Float maxt) const {
  NotImplementedError("rayIntersect");
}

void Shape::fillIntersectionRecord(const Ray &ray, const void *temp, Intersection &its) const {
  NotImplementedError("fillIntersectionRecord");
}

void Shape::getCurvature(const Intersection &its, Float &H, Float &K, bool shadingFrame) const {
  Vector dndu, dndv;
  getNormalDerivative(its, dndu, dndv, shadingFrame);

  /* Compute the coefficients of the first and second fundamental form */
  Float E = dot(its.dpdu, its.dpdu), F = dot(its.dpdu, its.dpdv), G = dot(its.dpdv, its.dpdv),
        e = -dot(its.dpdu, dndu), f = -dot(its.dpdv, dndu), g = -dot(its.dpdv, dndv),
...
void Shape::getNormalDerivative(const Intersection &its, Vector &dndu, Vector &dndv,
...
ref<TriMesh> Shape::createTriMesh() { return NULL; }
