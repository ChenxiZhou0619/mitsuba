...
void GradientPathTracer::evaluatePoint(
  ...
  while (main.rRec.depth < m_config->m_maxDepth || m_config->m_maxDepth < 0) {
    ...
    // Sample incoming radiance from lights (next event estimation).
    {
      ...
      if (main.rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
        ...
        // it.
        Float mainBsdfPdf = (emitter->isOnSurface() &&
                             dRec.measure == ESolidAngle && mainEmitterVisible)
        ...
        Float mainOpposingCosine =
            dot(dRec.n, (main.rRec.its.p - dRec.p)) / sqrt(mainDistanceSquared);
        ...
        if (shifted.connection_status == RAY_CONNECTED) {
          ...
          if (mainVertexType == VERTEX_TYPE_DIFFUSE &&
            ...
            if (!lastSegment || mainHitEmitter ||
              ...
              BSDFSamplingRecord bRec(
                  shifted.rRec.its, shifted.rRec.its.toLocal(incomingDirection),
            ...
            // ! This might be the precision problems
            // SAssert(fabs(shifted.ray.d.lengthSquared() - 1) < 0.000001);
            SAssert(fabs(shifted.ray.d.lengthSquared() - 1) < 0.00001);

            // Apply the local shift.
            HalfVectorShiftResult shiftResult =
                halfVectorShift(mainBsdfResult.bRec.wi, mainBsdfResult.bRec.wo,
                                shifted.rRec.its.toLocal(-shifted.ray.d),
                                mainBSDF->getEta(), shiftedBSDF->getEta());
          ...
          half_vector_shift_failed:
            if (shifted.alive) {
              ...
              // type.
              weight =
                  main.pdf / (shifted.pdf * shifted.pdf + main.pdf * main.pdf);
              mainContribution = main.throughput * mainEmitterRadiance;
              shiftedContribution =
                  shifted.throughput *
                  shiftedEmitterRadiance; // Note: Jacobian baked into
