 ...
 */
static IOREntry iorData[] = {{"vacuum", 1.0f},
                             {"helium", 1.000036f},
                             {"hydrogen", 1.000132f},
                             {"air", 1.000277f},
...
static Float lookupIOR(const std::string &name) {
  ...
  while (ior->name) {
    if (lowerCase == ior->name) return ior->value;
    ++ior;
  }

  std::ostringstream oss;
  oss << "Unable to find an IOR value for \"" << lowerCase << "\"! Valid choices are:";
...
inline Float lookupIOR(const Properties &props, const std::string &paramName,
