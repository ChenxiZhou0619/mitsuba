 ...
 * situations where a light ray impinges on an object from a direction that is classified as
 * ``outside'' according to the shading normal, and ``inside'' according to the true geometric
 * normal.
 *
 * The \code{strictNormals}
 * parameter specifies the intended behavior when such cases arise. The default (\code{false}, i.e.
 * ``carry on'') gives precedence to information given by the shading normal and considers such
 * light paths to be valid. This can theoretically cause light ``leaks'' through boundaries, but it
 * is not much of a problem in practice.
 *
 * When set to \code{true}, the path tracer detects inconsistencies and ignores these paths. When
 * objects are poorly tesselated, this latter option may cause them to lose a significant amount of
 * the incident radiation (or, in other words, they will look dark).
 *
 * The bidirectional integrators in Mitsuba (\pluginref{bdpt}, \pluginref{pssmlt}, \pluginref{mlt}
 * ...) implicitly have \code{strictNormals} set to \code{true}. Hence, another use of this
 * parameter is to match renderings created by these methods.
 *
 * \remarks{
...
public:
  ...
  MIPathTracer(Stream *stream, InstanceManager *manager) : MonteCarloIntegrator(stream, manager) {}
