...
class VPMIntegrator : public Integrator {
public:
  ...
  VPMIntegrator(const Properties &props) : Integrator(props) {
  ...
  VPMIntegrator(Stream *stream, InstanceManager *manager)
    ...
    for (int yofs = 0; yofs < cropSize.y; yofs += blockSize) {
      for (int xofs = 0; xofs < cropSize.x; xofs += blockSize) {
        ...
        std::vector<GatherPoint> &gatherPoints =
            m_gatherBlocks[m_gatherBlocks.size() - 1];
  ...
  void distributedRTPass(Scene                             *scene,
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int i = 0; i < (int)m_gatherBlocks.size(); ++i) {
...
#if defined(MTS_OPENMP)
      Sampler *sampler =
          static_cast<Sampler *>(samplers[mts_omp_get_thread_num()]);
#else
      ...
      for (int yofsInt = 0; yofsInt < blockSize; ++yofsInt) {
        if (yofsInt + yofs - cropOffset.y >= cropSize.y) continue;
        for (int xofsInt = 0; xofsInt < blockSize; ++xofsInt) {
          if (xofsInt + xofs - cropOffset.x >= cropSize.x) continue;
          ...
          if (needsApertureSample) apertureSample = sampler->next2D();
          if (needsTimeSample) timeSample = sampler->next1D();
          sample = sampler->next2D();
          sample += Vector2((Float)gatherPoint.pos.x, (Float)gatherPoint.pos.y);
          RayDifferential ray;
          sensor->sampleRayDifferential(ray, sample, apertureSample,
                                        timeSample);
          ...
          while (true) {
            if (scene->rayIntersect(ray, gatherPoint.its)) {
              ...
              if ((bsdf->getType() & BSDF::EAll) == BSDF::EDiffuseReflection ||
                  (bsdf->getType() & BSDF::EAll) ==
                      BSDF::EDiffuseTransmission ||
                  (depth + 1 > m_maxDepth && m_maxDepth != -1)) {
                gatherPoint.weight = weight;
                gatherPoint.depth  = depth;
                break;
              } else {
                /* Recurse for dielectric materials and (specific to VPM):
                ...
                ray =
                    RayDifferential(gatherPoint.its.p,
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int blockIdx = 0; blockIdx < (int)m_gatherBlocks.size(); ++blockIdx) {
      ...
      for (size_t i = 0; i < gatherPoints.size(); ++i) {
        ...
        if (N == 0 && !gp.emission.isZero()) gp.N = N = 1;
        ...
        if (N + M == 0) {
          ...
          gp.flux = (gp.flux + gp.weight * flux +
                     gp.emission * (Float)proc->getShotParticles() * M_PI *
                         gp.radius * gp.radius) *
                    ratio;
          gp.N = N + m_alpha * M;
          contrib =
              gp.flux / ((Float)m_totalEmitted * gp.radius * gp.radius * M_PI);
        }
...
MTS_IMPLEMENT_CLASS_S(VPMIntegrator, false, SamplingIntegrator)
MTS_EXPORT_PLUGIN(VPMIntegrator, "Volumetri Photon map integrator");
MTS_NAMESPACE_END
