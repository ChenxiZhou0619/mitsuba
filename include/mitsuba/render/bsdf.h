...
public:
   ...
   *
   */
  inline BSDFSamplingRecord(const Intersection &its, const Vector &wi, const Vector &wo,
                            ETransportMode mode = ERadiance);
...
public:
  ...
  Float lambda;  // Use for wavelength dependent bsdf sampling
  int   channel; // Indicate the channel index
};
...
public:
  ...
  enum ETypeCombinations {
    ...
    ETransmission = EDiffuseTransmission | EDeltaTransmission | EDelta1DTransmission |
                    EGlossyTransmission | ENull,
  ...
  inline int getComponentCount() const { return (int)m_components.size(); }
   ...
   * \sa EBSDFType
   */
  inline unsigned int getType() const { return m_combinedType; }

  /**
   * Returns the classification flags of a specific component
   * \sa EBSDFType
   */
  inline unsigned int getType(int component) const { return m_components[component]; }
  ...
  inline bool hasComponent(unsigned int type) const { return (type & m_combinedType) != 0; }

  /// Return whether this BSDF makes use of ray differentials
  inline bool usesRayDifferentials() const { return m_usesRayDifferentials; }
  ...
  virtual Spectrum getSpecularReflectance(const Intersection &its) const { return Spectrum(0.0f); }
...
protected:
   ...
   */
  Texture *ensureEnergyConservation(Texture *tex, const std::string &paramName, Float max) const;
   ...
   * is abided) and prints a warning.
   */
  std::pair<Texture *, Texture *> ensureEnergyConservation(Texture *tex1, Texture *tex2,
