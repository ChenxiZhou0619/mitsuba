 ...
 * of light). As a consequence, participating media enclosed by a stencil shape
 * (see \secref{shapes} for details) are rendered considerably more efficiently
 * when this shape has \emph{no}\footnote{this is what signals to Mitsuba that
 * the boundary is index-matched and does not interact with light in any way.
 * Alternatively, the \pluginref{mask} and \pluginref{thindielectric} BSDF can
 * be used to specify index-matched boundaries that involve some amount of
 * interaction.} BSDF assigned to it (as compared to, say, a
 * \pluginref{dielectric} or \pluginref{roughdielectric} BSDF).
 *
 * \remarks{
...
public:
  ...
  Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
    ...
    while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
      ...
      /* ==================================================================== */
      if (rRec.medium &&
          rRec.medium->sampleDistance(Ray(ray, 0, its.t), mRec, rRec.sampler)) {
        /* Sample the integral
           \int_x^y tau(x, x') [ \sigma_s \int_{S^2} \rho(\omega,\omega')
           L(x,\omega') d\omega' ] dx'
        */
        const PhaseFunction *phase = mRec.getPhaseFunction();

        if (rRec.depth >= m_maxDepth &&
            m_maxDepth != -1) // No more scattering events allowed
          break;
        ...
        if (rRec.type & RadianceQueryRecord::EDirectMediumRadiance) {
          ...
          Spectrum value = scene->sampleAttenuatedEmitterDirect(
              dRec, rRec.medium, interactions, rRec.nextSample2D(),
              rRec.sampler);
        ...
        rayIntersectAndLookForEmitter(scene, rRec.sampler, rRec.medium,
                                      m_maxDepth - rRec.depth - 1, ray, its,
                                      dRec, value);

        /* If a luminaire was hit, estimate the local illumination and
           weight using the power heuristic */
        if (!value.isZero() &&
            (rRec.type & RadianceQueryRecord::EDirectMediumRadiance)) {
        ...
        /* Sample
            tau(x, y) (Surface integral). This happens with probability
           mRec.pdfFailure Account for this and multiply by the proper
           per-color-channel transmittance.
        */
        if (rRec.medium)
        ...
        if ((rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance) &&
          ...
          Spectrum value = scene->sampleAttenuatedEmitterDirect(
              dRec, its, rRec.medium, interactions, rRec.nextSample2D(),
              rRec.sampler);
  ...
  void rayIntersectAndLookForEmitter(const Scene *scene, Sampler *sampler,
                                     const Medium *medium, int maxInteractions,
                                     Ray ray, Intersection &_its,
    ...
    while (true) {
      ...
      if (medium)
        transmittance *=
            medium->evalTransmittance(Ray(ray, 0, its->t), sampler);
