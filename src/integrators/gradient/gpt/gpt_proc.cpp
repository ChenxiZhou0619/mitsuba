...
public:
  GPTBlockRenderer(Bitmap::EPixelFormat pixelFormat, int channelCount,
                   int blockSize, int borderSize, bool warnInvalid)
      : m_pixelFormat(pixelFormat), m_channelCount(channelCount),
  ...
  ref<WorkUnit> createWorkUnit() const { return new RectangularWorkUnit(); }

  ref<WorkResult> createWorkResult() const {
    return new GPTWorkResult(m_sensor->getFilm()->getReconstructionFilter(),
                             Vector2i(m_blockSize), 1);
  }
  ...
  void prepare() {
    ...
    m_integrator =
        static_cast<GradientPathIntegrator *>(getResource("integrator"));
  ...
  void process(const WorkUnit *workUnit, WorkResult *workResult,
               const bool &stop) {
    const RectangularWorkUnit *rect =
        static_cast<const RectangularWorkUnit *>(workUnit);
    ...
    m_integrator->renderBlock(m_scene, m_sensor, m_sampler, block, stop,
                              m_hilbertCurve.getPoints());
  ...
  ref<WorkProcessor> clone() const {
    return new GPTBlockRenderer(m_pixelFormat, m_channelCount, m_blockSize,
                                m_borderSize, m_warnInvalid);
  }
...
GPTRenderProcess::GPTRenderProcess(const RenderJob *parent, RenderQueue *queue,
                                   int blockSize,
                                   const GradientPathTracerConfig &config)
    : BlockedRenderProcess(parent, queue, blockSize), m_config(config) {}
