...
template <typename _PointType, typename _VectorType> struct TRay {
  ...
  uint32_t heroChannel;

  /// Construct a new ray
  inline TRay()
      : mint(Epsilon), maxt(std::numeric_limits<Scalar>::infinity()), time(0) {}

  /// Copy constructor (1)
  inline TRay(const TRay &ray)
      : o(ray.o), mint(ray.mint), d(ray.d), maxt(ray.maxt), dRcp(ray.dRcp),
        time(ray.time), heroChannel(ray.heroChannel) {}

  /// Copy constructor (2)
  inline TRay(const TRay &ray, Scalar mint, Scalar maxt)
      : o(ray.o), mint(mint), d(ray.d), maxt(maxt), dRcp(ray.dRcp),
        time(ray.time), heroChannel(heroChannel) {}

  /// Construct a new ray, while not specifying a direction yet
  inline TRay(const PointType &o, Scalar time)
      : o(o), mint(Epsilon), maxt(std::numeric_limits<Scalar>::infinity()),
        time(time) {}
  ...
  inline std::string toString() const {
    ...
    oss << "Ray[origin=" << o.toString() << ", direction=" << d.toString()
        << ", mint=" << mint << ", maxt=" << maxt << ", time=" << time << "]";
    return oss.str();
  }
};
...
struct RayDifferential : public Ray {
  ...
  inline RayDifferential() : hasDifferentials(false) {}
