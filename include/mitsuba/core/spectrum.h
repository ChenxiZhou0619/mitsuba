...
public:
   ...
   */
  inline BlackBodySpectrum(Float temperature) { m_temperature = temperature; }
...
public:
  ...
  explicit inline TSpectrum(Scalar spec[N]) { memcpy(s, spec, sizeof(Scalar) * N); }

  /// Unserialize a spectral power distribution from a binary data stream
  explicit inline TSpectrum(Stream *stream) { stream->readArray(s, N); }
  ...
  /// Multiply by a scalar
  inline friend TSpectrum operator*(Scalar f, const TSpectrum &spec) { return spec * f; }
  ...
  inline TSpectrum operator/(Scalar f) const {
    TSpectrum value = *this;
#ifdef MTS_DEBUG
    if (f == 0) SLog(EWarn, "TSpectrum: Division by zero!");
#endif
    Scalar recip = 1.0f / f;
    for (int i = 0; i < N; i++)
      value.s[i] *= recip;
  ...
  inline bool operator==(const TSpectrum &spec) const {
    for (int i = 0; i < N; i++) {
      if (s[i] != spec.s[i]) return false;
    }
    return true;
  }

  /// Inequality test
  inline bool operator!=(const TSpectrum &spec) const { return !operator==(spec); }

  /// Divide by a scalar
  inline friend TSpectrum operator/(Scalar f, TSpectrum &spec) { return TSpectrum(f) / spec; }

  /// Divide by a scalar
  inline TSpectrum &operator/=(Scalar f) {
#ifdef MTS_DEBUG
    if (f == 0) SLog(EWarn, "TTSpectrum: Division by zero!");
#endif
    Scalar recip = 1.0f / f;
    for (int i = 0; i < N; i++)
      s[i] *= recip;
  ...
  inline bool isValid() const {
    for (int i = 0; i < N; i++)
      if (!std::isfinite(s[i]) || s[i] < 0.0f) return false;
    return true;
  }
  ...
  /// Indexing operator
  inline Scalar &operator[](int entry) { return s[entry]; }

  /// Indexing operator
  inline Scalar operator[](int entry) const { return s[entry]; }

  /// Check if this spectrum is zero at all wavelengths
  inline bool isZero() const {
    for (int i = 0; i < N; i++) {
      if (s[i] != 0.0f) return false;
    }
    return true;
  }

  /// Serialize this spectrum to a stream
  inline void serialize(Stream *stream) const { stream->writeArray(s, N); }
...
public:
  ...
  inline bool operator==(const Spectrum &val) const {
    for (int i = 0; i < SPECTRUM_SAMPLES; i++) {
      if (s[i] != val.s[i]) return false;
    }
    return true;
  }

  /// Inequality test
  inline bool operator!=(const Spectrum &val) const { return !operator==(val); }
