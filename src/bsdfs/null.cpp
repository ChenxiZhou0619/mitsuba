...
public:
  Null(const Properties &props) : BSDF(props) {}

  Null(Stream *stream, InstanceManager *manager) : BSDF(stream, manager) { configure(); }
  ...
  Float getRoughness(const Intersection &its, int component) const { return 0.0f; }
...
public:
  NullShader(Renderer *renderer) : Shader(renderer, EBSDFShader) { m_flags = ETransparent; }
...
Shader *Null::createShader(Renderer *renderer) const { return new NullShader(renderer); }
