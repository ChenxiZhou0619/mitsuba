...
Medium::Medium(const Properties &props) : NetworkedObject(props) {
  Spectrum g;
...
Medium::Medium(Stream *stream, InstanceManager *manager) : NetworkedObject(stream, manager) {
...
void Medium::addChild(const std::string &name, ConfigurableObject *child) {
  ...
  if (cClass->derivesFrom(MTS_CLASS(PhaseFunction))) {
    ...
    Log(EError, "Medium: Invalid child node! (\"%s\")", cClass->getName().c_str());
  }
}

void Medium::configure() {
  if (m_phaseFunction == NULL) {
    m_phaseFunction = static_cast<PhaseFunction *>(PluginManager::getInstance()->createObject(
        MTS_CLASS(PhaseFunction), Properties("isotropic")));
...
//* MajorantSamplingRecord
const PhaseFunction *MajorantSamplingRecord::getPhaseFunction() const {
